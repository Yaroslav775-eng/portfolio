...
# --- ENGLISH ---
en_cards = [
    ("E-commerce: Pet Supplies", "Architecture", "Replaced chaotic catalog spend with structured demand capture layers. Isolated high-value Search from broad Shopping for predictable ROI.", "800-1200%", "ROAS", "€4.35", "CPA", 1, "System View"),
    ("E-commerce: BBQ Grills", "Scaling", "Priority-driven category scaling. Optimized inventory exposure using PMax for reach and Search for capturing high-margin intent.", "679%", "ROAS", "€13.7", "CPA", 2, "System View"),
    ("B2B: Logistics", "LeadGen", "Isolated B2B intent from generic volume. Built a transparent lead-gen system with brand isolation and DSA gap filling.", "€11.5", "Target CPL", "1,090", "Leads", 3, "System View"),
    ("E-commerce: Electronics", "Technical", "Tiered inventory scaling. Strengthened PMax visibility for top-performing product groups while preserving branded Search control.", "653%", "ROAS", "€6.7", "CPA", 4, "System View"),
...
# --- RUSSIAN ---
ru_cards = [
    ("E-commerce: зоотовары", "Архитектура", "Трансформировали хаотичные траты в систему управления спросом. Изолировали прибыльный Search от широкого Shopping для стабилизации ROAS.", "800-1200%", "ROAS", "€4.35", "CPA", 1, "Системный Вид"),
    ("E-commerce: товары для гриля", "Масштабирование", "Масштабирование категорий на основе приоритетов. Оптимизировали охват инвентаря: PMax для емкости и Search для маржинального спроса.", "679%", "ROAS", "€13.7", "CPA", 2, "Системный Вид"),
    ("B2B: Логистика", "Лидогенерация", "Изолировали B2B-интент от общего объема. Построили прозрачную систему лидогенерации с изоляцией бренда и DSA-наполнением.", "€11.5", "CPL (Цена Лида)", "1,090", "Лидов", 3, "Системный Вид"),
    ("E-commerce: электротовары", "Технический", "Многоуровневое масштабирование инвентаря. Усилили видимость PMax для топовых групп при контроле брендового поиска.", "653%", "ROAS", "€6.7", "CPA", 4, "Системный Вид"),
    ...
    ("Industrial B2B: Оборудование", "Стратегия", "Supply-chain фильтрация интента. Жесткая логика исключения розничного шума для захвата профессиональных покупателей.", "320", "Лидов", "€45", "CPA", 6, "Системный Вид")
]
ru_grid = '<div class="cases-grid">' + "".join([gen_grid_card(*c) for c in ru_cards])
ru_modals = '<div style="display: none;">' + \
    gen_modal("E-commerce: зоотовары", "Пересобрать архитектуру аккаунта для приоритета подтвержденного результата над широким охватом каталога.", "Стратегическая архитектура", "Распределение ролей: Search (сформированный спрос), Shopping (каталог), PMax (масштабирование). Изоляция категорий «Собаки/Кошки» для точечного управления.", "funnel", "Интеграция Search + PMax", "Техническая реализация", ["Анализ выявил перерасход в широком Shopping. Бюджет переведен в Search-слои.", "Изоляция бренда от общего шума архитектуры.", "Точечное управление приоритетами (Собаки/Кошки) вместо работы со всей массой ассортимента."], "Результаты", "800-1200%", "ROAS", "€4.35", "Avg. CPA", "34 923", "Конверсий", 1) + \
    gen_modal("E-commerce: товары для гриля", "Масштабировать приоритетные категории в пик сезона.", "Стратегическая архитектура", "Приоритетное масштабирование: Фокус на маржу в PMax.", "growth", "Динамика роста", "Техническая реализация", ["Пересборка под маржинальные группы.", "DSA для сезонного спроса.", "Уточнение отслеживания событий."], "Результаты", "679%", "ROAS", "€13.7", "Avg. CPA", "€157.6k", "Value", 2) + \
    gen_modal("B2B: Логистика", "Отсечь розничный (B2C) шум и стабилизировать заявки.", "Стратегическая архитектура", "B2B-фильтр интента: Глубокая логика минус-слов.", "funnel", "B2B Лид-воронка", "Техническая реализация", ["Изоляция бренда.", "DSA для низкочастотных услуг.", "Оптимизация воронки лида."], "Результаты", "€11.5", "Target CPL (Цена лида)", "1,090", "B2B Лидов", "€10.3k", "Расход", 3) + \
    gen_modal("E-commerce: электротовары", "Повысить управляемость на большом массиве SKU.", "Стратегическая архитектура", "ROAS-стратегия: Группировка товаров по эффективности.", "growth", "Масштабирование SKU", "Техническая реализация", ["Выделение приоритетов.", "PMax + Search для топов.", "Ремаркетинг для корзин."], "Результаты", "653%", "ROAS", "€6.7", "Avg. CPA", "€51.6k", "Value", 4) + \
    ...
    gen_modal("Industrial B2B: Оборудование", "Создать воронку под цепочки поставок.", "Стратегическая архитектура", "Supply-Chain фокус: Оптовые и индустриальные кластеры.", "funnel", "Фильтрация качества", "Техническая реализация", ["Изоляция профи-запросов.", "Ремаркетинг на прайс-листы.", "Отслеживание B2B-форм."], "Результаты", "320", "Квал. лидов", "€45", "Target CPA", "€14.4k", "Расход", 6) + \
    "</div>"

# --- UKRAINIAN ---
ua_cards = [
    ("E-commerce: зоотовари", "Архітектура", "Трансформували хаотичні витрати в систему управління попитом. Ізолювали прибутковий Search від широкого Shopping для стабілізації ROAS.", "800-1200%", "ROAS", "€4.35", "CPA", 1, "Системний Вигляд"),
    ("E-commerce: товари для гриля", "Масштабування", "Масштабування категорій на основі пріоритетів. Оптимізували охоплення інвентарю: PMax для ємності та Search для маржинального попиту.", "679%", "ROAS", "€13.7", "CPA", 2, "Системний Вигляд"),
    ("B2B: Логістика", "Лідогенерація", "Ізолювали B2B-інтент від загального обсягу. Побудували прозову систему лідогенерації з ізоляцією бренду та DSA-наповненням.", "€11.5", "CPL (Ціна Ліда)", "1,090", "Лідів", 3, "Системний Вигляд"),
    ("E-commerce: електротовари", "Технічний", "Багаторівневе масштабування інвентарю. Посилили видимість PMax для топових груп при контролі брендового пошуку.", "653%", "ROAS", "€6.7", "CPA", 4, "Системний Вигляд"),
...
ua_modals = '<div style="display: none;">' + \
    gen_modal("E-commerce: зоотовари", "Трансформувати хаотичний акаунт у прозору систему пріоритезації підтвердженого прибутку.", "Стратегічна архітектура", "Розподіл ролей: Search (Гарячий попит), Shopping (Каталог), PMax (Масштабування). Сегментація Собаки/Коти в ізольовані кампанії.", "funnel", "Інтеграція Search + PMax", "Технічна реалізація", ["Аналіз виявив перевитрати у широкому Shopping. Бюджет переведений у Search-шари.", "Ізоляція брендового попиту для запобігання розмиттю маржі.", "Точкове управління категоріями (Собаки/Коти) замість роботи з усією масою асортименту."], "Результати", "800-1200%", "ROAS", "€4.35", "Avg. CPA", "34 923", "Конверсій", 1) + \
    gen_modal("E-commerce: товари для гриля", "Масштабувати пріоритетні категорії у пік сезону.", "Стратегічна архітектура", "Пріоритетне масштабування: Фокус на маржу в PMax.", "growth", "Динаміка росту", "Технічна реалізація", ["Перезбирання під маржинальні групи.", "DSA для сезонного попиту.", "Уточнення відстеження подій."], "Результати", "679%", "ROAS", "€13.7", "Avg. CPA", "€157.6k", "Value", 2) + \
    gen_modal("B2B: Логістика", "Відсікти роздрібний (B2C) шум та стабілізувати заявки.", "Стратегічна архітектура", "B2B-фільтр інтенту: Глибока логіка мінус-слів.", "funnel", "B2B Лід-воронка", "Технічна реалізація", ["Ізоляція бренду.", "DSA для низькочастотних послуг.", "Оптимізація воронки ліда."], "Результати", "€11.5", "Target CPL (Ціна ліда)", "1,090", "B2B Лідів", "€10.3k", "Витрати", 3) + \
    gen_modal("E-commerce: електротовари", "Підвищити керованість на великому масиві SKU.", "Стратегічна архітектура", "ROAS-стратегія: Групування товарів за ефективністю.", "growth", "Масштабування SKU", "Технічна реалізація", ["Виділення пріоритетів.", "PMax + Search для топів.", "Ремаркетинг для кошиків."], "Результаты", "653%", "ROAS", "€6.7", "Avg. CPA", "€51.6k", "Value", 4) + \
...
print("All language versions updated successfully with visual anchors and ROAS/CPL metrics.")
